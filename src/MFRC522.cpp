...
void MFRC522::PICC_DumpToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	switch (piccType) {
		...
		case PICC_TYPE_MIFARE_4K:
			...
			PICC_DumpMifareClassicToSerial(uid, piccType, &key); //ici
			break;
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
			int32_t value = (int32_t(buffer[3])<<24) | (int32_t(buffer[2])<<16) | (int32_t(buffer[1])<<8) | int32_t(buffer[0]); // valeur du solde
			Serial.print(F(" Value=0x")); Serial.print(value, HEX); //Affiche Valeur du solde
			Serial.print(F(" Adr=0x")); Serial.print(buffer[12], HEX); //Affice Adresse du solde
		}
		Serial.println();
	}
